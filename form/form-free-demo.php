...
//------------------------------------- BASIC CONFIGURATION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   *************************************************************************************************/
   $file_ext = array('doc','docx','pdf','pps','ppt','xls','csv','png','jpg','gif','bmp');

  /*************************************************************************************************
   ** FROM y TO                                                                                    *
   *************************************************************************************************/
  define('DEX_SEND_EMAIL_FROM', 'info@rynfile.com');
  define('DEX_FIX_DESTINATION_EMAIL', 'cadie@dreamfactoryagency.com');

  /*************************************************************************************************
   ** Email subject, you can change it here                                                        *
   *************************************************************************************************/
  $emailsubject = 'RYN Free Demo Form Submission';
  ...
  /*************************************************************************************************
   ** Select to include or not, the additional information and other features                      *
   *************************************************************************************************/
  define('DEX_INCLUDE_ADDITIONAL_INFORMATION', true);
  define('DEX_SEND_EMAIL_COPY', true);
  define('DEX_SEND_EMAIL_ATTACHMENTS', true);
  define('DEX_SAVE_EXCEL_FILE', true);
  define('DEX_EMAIL_EMPTY_FIELDS', true);


  /*************************************************************************************************
   ** CSV file configuration                                                                       *
   *************************************************************************************************/
  define('DEX_CSV_FILE_FIELD_DELIMITER', ',');
  define('DEX_FOLDER_POSTFIX', '002');
  define('DEX_UPLOAD_FOLDER', 'saved_forms');
...
//------------------------------------- MAIN CODE STARTS HERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // ********************************************************************
  // MAIN CODE / DON'T EDIT AFTER THIS LINE
  // ********************************************************************
  ...
  define ("FOLDER_NAME",substr(str_replace("\\","/",dirname($_SERVER["PHP_SELF"])),1)."/saved_forms");
  ...
  foreach ($_FILES as $item => $value)  //extension valid
      if (!isValidExt($value['name'])) { header ("Location: ".getHDWForm2MailParam(HDW_F2M_NO_OK)); exit; }
  function isValidExt($value)
  {
      global $file_ext;
      if ($value!="")
      {
          $pos = strrpos($value,"."); $ext = substr($value,$pos+1);
          if(in_array($ext, $file_ext)) return true; else return false;
      }
      return true;
  }
  ...
  if (DEX_SAVE_EXCEL_FILE)
  {
      // To Upload Folder
      if (DEX_FOLDER_POSTFIX == "") $uploadfolder= DEX_UPLOAD_FOLDER."_".substr(md5($_SERVER["SERVER_NAME"]),0,4);
      else $uploadfolder= DEX_UPLOAD_FOLDER."_".DEX_FOLDER_POSTFIX;
      $dc = __FILE__;
      if ($dc == '')
      ...
      $documentroot  = $dc;
      if (DEX_INCLUDE_ADDITIONAL_INFORMATION) define('FIXED',6); else define('FIXED',0);
      ...
      if (!file_exists(DATA_UPLOAD_MAIN))
         if (!(mkdir(DATA_UPLOAD_MAIN,0777)))
         {
      ...
      foreach ($_GET as $item => $value)
        if ($item != HDW_F2M_OK && $item != FOLDER_NAME && $item != HDW_F2M_NO_OK && notInThisArray($item))
            $formfields[$item] = str_replace("\'","'",(is_array($value)?implode(", ",$value):$value));
      ...
      {
        if ($value['name']!="" && notInThisArray($item))
        {
      ...
      if (file_exists(DATA_FILE))
      {
         ...
         $headers = fgetcsv($fd,$maxsize,DEX_CSV_FILE_FIELD_DELIMITER);
         $headers2 = array();
         $ckheaders = count($headers)-FIXED;
         foreach ($formfields as $item => $value) {
            $located = false;
            for ($i=0;($i< $ckheaders) && (!$located);$i++)
            {
                if ($item == $headers[$i])
                    $located = true;
            }
         ...
         for ($i=0;$i<$ckheaders;$i++)
         {
             if (isset($formfields[$headers[$i]]))
                 $values .= '"'.str_replace('"','""',$formfields[$headers[$i]]).'"'.DEX_CSV_FILE_FIELD_DELIMITER;
             else
                 $values .= DEX_CSV_FILE_FIELD_DELIMITER;
             $head .= $headers[$i].DEX_CSV_FILE_FIELD_DELIMITER;
         ...
         if (count($headers2)==0)
         {
            ...
            if (DEX_INCLUDE_ADDITIONAL_INFORMATION) $values .= $countryname["x".getCountryID($_SERVER['REMOTE_ADDR'])].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['REMOTE_ADDR'].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER["HTTP_REFERER"].DEX_CSV_FILE_FIELD_DELIMITER.date("Y-m-d H:i:s").DEX_CSV_FILE_FIELD_DELIMITER.ckbrowser($_SERVER['HTTP_USER_AGENT']).DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['HTTP_USER_AGENT'].DEX_CSV_FILE_FIELD_DELIMITER;
         ...
         else
         {
            for ($i=0;$i<count($headers2);$i++)
                $ins .= DEX_CSV_FILE_FIELD_DELIMITER;

            $contents = "";

            while ($v = fgetcsv($fd,$maxsize,DEX_CSV_FILE_FIELD_DELIMITER))
            {
              //if (!feof ($fd))
              {
                  for ($i=0;$i<$ckheaders;$i++)
                      $contents .= '"'.str_replace('"','""',$v[$i]).'"'.DEX_CSV_FILE_FIELD_DELIMITER; //$contents .= $v[$i].',';
                  $contents .= $ins;
                  for ($i=$ckheaders;$i<($ckheaders+FIXED);$i++)
                      $contents .= '"'.str_replace('"','""',$v[$i]).'"'.DEX_CSV_FILE_FIELD_DELIMITER; //$contents .= $v[$i].',';
            ...
            for ($i=0;$i<count($headers2);$i++)
            {
                $head .= '"'.str_replace('"','""',$headers2[$i]).'"'.DEX_CSV_FILE_FIELD_DELIMITER;
                $values .= '"'.str_replace('"','""',$formfields[$headers2[$i]]).'"'.DEX_CSV_FILE_FIELD_DELIMITER;
            }
            for ($i=$ckheaders;$i<($ckheaders+FIXED);$i++)
                $head .= '"'.str_replace('"','""',$headers[$i]).'"'.DEX_CSV_FILE_FIELD_DELIMITER; //$headers[$i].',';
            if (DEX_INCLUDE_ADDITIONAL_INFORMATION) $values .= $countryname["x".getCountryID($_SERVER['REMOTE_ADDR'])].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['REMOTE_ADDR'].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER["HTTP_REFERER"].DEX_CSV_FILE_FIELD_DELIMITER.date("Y-m-d H:i:s").DEX_CSV_FILE_FIELD_DELIMITER.ckbrowser($_SERVER['HTTP_USER_AGENT']).DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['HTTP_USER_AGENT'].DEX_CSV_FILE_FIELD_DELIMITER;
      ...
      else
      {
        ...
        foreach ($formfields as $item => $value)
        {
            $head .= '"'.str_replace('"','""',$item).'"'.DEX_CSV_FILE_FIELD_DELIMITER;
            $values .= '"'.str_replace('"','""',$value).'"'.DEX_CSV_FILE_FIELD_DELIMITER;
        }
        if (DEX_INCLUDE_ADDITIONAL_INFORMATION) $head .= "Country".DEX_CSV_FILE_FIELD_DELIMITER."IP".DEX_CSV_FILE_FIELD_DELIMITER."Referer".DEX_CSV_FILE_FIELD_DELIMITER."Server Time".DEX_CSV_FILE_FIELD_DELIMITER."Browser".DEX_CSV_FILE_FIELD_DELIMITER."User Agent".DEX_CSV_FILE_FIELD_DELIMITER;
        if (DEX_INCLUDE_ADDITIONAL_INFORMATION) $values .= $countryname["x".getCountryID($_SERVER['REMOTE_ADDR'])].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['REMOTE_ADDR'].DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER["HTTP_REFERER"].DEX_CSV_FILE_FIELD_DELIMITER.date("Y-m-d H:i:s").DEX_CSV_FILE_FIELD_DELIMITER.ckbrowser($_SERVER['HTTP_USER_AGENT']).DEX_CSV_FILE_FIELD_DELIMITER.$_SERVER['HTTP_USER_AGENT'].DEX_CSV_FILE_FIELD_DELIMITER;
  ...
  if (!DEX_SEND_EMAIL_COPY)  // if no email copy required then finished here
  {
  ...
  function send_mail($emailaddress, $fromaddress, $emailsubject, $body, $attachments=false)
  {
      ...
      if ($attachments != false && DEX_SEND_EMAIL_ATTACHMENTS)
      {
  ...
  foreach ($_GET as $item => $value)
    if ($item != HDW_F2M_OK && $item != FOLDER_NAME && $item != HDW_F2M_NO_OK && notInThisArray($item) && (DEX_EMAIL_EMPTY_FIELDS || $value != ""))
  ...
  $body .="<strong>SUPPORT INFORMATION</strong><br />"
        ...
        ."<hr />Delivered by CodePeople Advanced Form2Excel";
